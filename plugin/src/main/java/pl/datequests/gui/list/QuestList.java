...
public class QuestList extends PluginGUI {
    ...
    public void onOpen() {
        ...
        List<QuestSchema> schemas = new ArrayList<>(DateQuests.getInstance().getQuestsManager().getQuestSchemas());
        this.slots = new ArrayList<>();
        int numberOfSchemas = schemas.size();
        if(numberOfSchemas == 0) {
            setSlot(22, Material.BARRIER, DateQuests.getInstance().getMessagesManager().getMessage("noAvailableQuests"),
                    getLore(DateQuests.getInstance().getMessagesManager().getMessage("noAvailableQuestsLore")));
            return;
